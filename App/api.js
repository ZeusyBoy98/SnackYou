...
const cameraConnectButton = document.getElementById("cameraConnect");
...
function connectCamera() {
  ...
  // The hosted HTTPS app cannot embed the ESP32's plain HTTP LAN stream.
  // The ESP32 pushes frames to the API, and this endpoint relays them as MJPEG.
  cameraFeed.src = `${BASEURL}/camera/stream?t=${Date.now()}`;
...
cameraFeed.onerror = () => {
  ...
  // Retry so the feed comes back automatically if the ESP32 reboots.
  clearTimeout(cameraRetryTimer);
  cameraRetryTimer = setTimeout(connectCamera, 3000);
};

cameraConnectButton.onclick = () => {
  connectCamera();
};
...
fireButton.onclick = () => {
  if (owner === id) {
    ...
    responseElement.textContent =
      "You have not got ownership of the lock. Gain ownership before attempting to fire the turret.";
  }
};
